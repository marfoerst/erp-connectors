...
export async function upsertCustomer(
  ...
  if (existing) {
    const personalAccount = await ensureDebitor(
      shop, client, existing, order, settings, isGuest,
    );
  ...
  if (email) {
    ...
    if (candidates.length === 1 && !isGuest) {
      ...
      if (matchesName(candidate, order)) {
        ...
        const personalAccount = await ensureDebitor(shop, client, candidate.id, order, settings, isGuest);
        await logEvent(shop, {
  ...
  const personalAccount = await ensureDebitor(shop, client, contactId, order, settings, isGuest);
