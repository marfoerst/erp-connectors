...
import type { OrderLike, ResolvedTaxTreatment } from "./types";
    ...
    for (const p of d.positions) {
      lines.push(
        [
          ...
          cell(d.documentDate),
          cell(d.orderGid),
          cell(d.orderName ? `Shopify ${d.orderName}` : ""),
          cell(d.currency),
          cell(d.country),
          cell(p.number),
          cell(p.name),
...
export async function createExportBatch(shop: string): Promise<ExportBatch> {
  ...
  await logEvent(shop, {
    ...
    data: { batchId, orders: drafts.map((d) => d.orderName ?? d.orderGid) },
  });
  ...
  return {
    ...
    batchId,
    count: drafts.length,
    orderNames: drafts.map((d) => d.orderName ?? d.orderGid),
  };
}
