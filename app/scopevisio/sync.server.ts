...
import { ScopevisioError } from "./client.server";
import { upsertCustomer } from "./contacts.server";
import {
  createInvoice,
  getInvoiceTaxCents,
  postInvoice,
...
import { TAX_CASE_LABEL } from "./constants";
...
export async function syncOrder(
  ...
  try {
    ...
    // 2. Customer.
    const customer = await upsertCustomer(shop, order, settings);
    ...
    const erpTaxCents = await getInvoiceTaxCents(shop, documentNumber);
    ...
    await postInvoice(shop, documentNumber);
