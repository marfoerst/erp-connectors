...
import { buildInvoiceXml, escapeXml } from "./invoices.server";
import { draftsToCsv } from "./csv-export.server";
import { buildPostings } from "./postings.server";
...
describe("buildPostings", () => {
  ...
  it("leaves exactly the VAT amount for autoCreateTax to generate", () => {
    ...
    const sum = p.rows.reduce((a, r) => a + r.amount, 0);
  ...
  it("puts the Shopify order id on every row for traceability", () => {
    ...
    for (const r of p.rows) expect(r.externalDocumentNumber).toBe(draft.orderGid);
  });
