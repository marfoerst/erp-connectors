...
import type { ConnectionRecord, ConnectionStore } from "./ports";
...
export class ScopevisioClient {
  private constructor(
    private conn: ConnectionRecord,
    /** Absent for an ephemeral client, which never persists anything. */
   ...
   */
  static async fromStore(store: ConnectionStore): Promise<ScopevisioClient> {
  ...
  static ephemeral(input: ConnectionInput): ScopevisioClient {
  ...
  private async persistTokens(token: TokenResponse) {
    ...
    const data: Partial<ConnectionRecord> = {
