...
export async function postInvoice(ctx: ScopevisioContext, documentNumber: string) {
  const client = ctx.client;
  return client.post(`/outgoinginvoice/${encodeURIComponent(documentNumber)}/post`);
}

export async function getInvoice(ctx: ScopevisioContext, documentNumber: string) {
  const client = ctx.client;
...
export async function getInvoiceTaxCents(
  ...
  try {
    const doc = (await getInvoice(ctx, documentNumber)) as Record<string, unknown>;
