...
export function taxChecksum(args: {
  /** Tax total the source system reported — a checksum, never an input. */
  ...
  const delta = Math.abs(erpTaxCents - args.sourceTaxCents);
  ...
  return {
    ...
    erpTaxCents,
    detail:
      `The shop calculated ${(args.sourceTaxCents / 100).toFixed(2)} tax, ` +
